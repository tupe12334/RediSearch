...
fn sort_weight_nested_intersection_sorts_first() {
    ...
    let inner_children: Vec<Box<dyn RQEIterator<'static> + 'static>> = (0..5)
        .map(|_| Box::new(IdListSorted::new(docs.clone())) as Box<dyn RQEIterator<'static> + 'static>)
        .collect();
    ...
    assert!(
        outer.child_at(0).is_intersection(),
