...
static size_t NI_NumEstimated(const QueryIterator *base) {
  const NotIteratorOptimized *ni = (const NotIteratorOptimized *)base;
...
QueryIterator *NewNotIterator(QueryIterator *it, t_docId maxDocId, double weight, struct timespec timeout, QueryEvalCtx *q) {
  ...
  if (optimized) {
    ...
    ret->current->docId = 0;
    ret->atEOF = false;
    ret->type = NOT_ITERATOR_OPTIMIZED;
    ret->lastDocId = 0;
...
QueryIterator const* GetNotIteratorChild(const QueryIterator *base) {
    if (base->type == NOT_ITERATOR_OPTIMIZED) {
        NotIteratorOptimized const*it = (const NotIteratorOptimized *)base;
        return it->child;
    } else {
        return GetNotIteratorNonOptimizedChild(base);
    }
}

void SetNotIteratorChild(QueryIterator *base, QueryIterator *newChild) {
    if (base->type == NOT_ITERATOR_OPTIMIZED) {
        NotIteratorOptimized *it = (NotIteratorOptimized *)base;
        if (it->child) {
            it->child->Free(it->child);
        }
        it->child = newChild;
    } else {
        SetNotIteratorNonOptimizedChild(base, newChild);
    }
}

QueryIterator *TakeNotIteratorChild(QueryIterator *base) {
    if (base->type == NOT_ITERATOR_OPTIMIZED) {
        NotIteratorOptimized *it = (NotIteratorOptimized *)base;
        ...
        return TakeNotIteratorNonOptimizedChild(base);
    }
}

void _SetNotIteratorOptimizedWildcard(QueryIterator *base, QueryIterator *wcii) {
    RS_ASSERT(base->type == NOT_ITERATOR_OPTIMIZED);
...
QueryIterator const* _GetNotIteratorOptimizedWildcard(const QueryIterator *base) {
    RS_ASSERT(base->type == NOT_ITERATOR_OPTIMIZED);
