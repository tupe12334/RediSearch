...
use ffi::{
    ...
    IteratorType_PROFILE_ITERATOR, IteratorType_WILDCARD_ITERATOR, QueryIterator,
...
/// Partial layout of the C `NotIteratorOptimized` struct, sufficient to
/// access the `child` field for profile wrapping.
#[repr(C)]
struct NotIteratorOptimized {
    base: QueryIterator,
    wcii: *mut QueryIterator,
    child: *mut QueryIterator,
    // remaining fields omitted — we only need access up to `child`.
}

/// Partial layout of the C `OptionalOptimizedIterator` struct, sufficient to
/// access the `child` field for profile wrapping.
#[repr(C)]
struct OptionalOptimizedIterator {
    base: QueryIterator,
    child: *mut QueryIterator,
    // remaining fields omitted — we only need access up to `child`.
}

/// Partial layout of the C `HybridIterator` struct, sufficient to
/// access the `child` field for profile wrapping.
#[repr(C)]
...
/// Partial layout of the C `OptimizerIterator` struct, sufficient to
/// access the `child` field for profile wrapping.
#[repr(C)]
struct OptimizerIterator {
    base: QueryIterator,
    child: *mut QueryIterator,
    // remaining fields omitted — we only need access up to `child`.
}

/// Partial layout of the C `UnionIterator` struct, sufficient to
/// access the `its_orig` and `num_orig` fields for profile wrapping.
#[repr(C)]
struct UnionIterator {
    base: QueryIterator,
    its_orig: *mut *mut QueryIterator,
    num_orig: u32,
    // remaining fields omitted — we only need access up to `num_orig`.
}
...
fn profile_child(child: CRQEIterator) -> CRQEIterator {
    ...
    let ptr = RQEIteratorWrapper::boxed_new(IteratorType_PROFILE_ITERATOR, profiled);
...
impl Profilable<'_> for CRQEIterator {
    ...
    fn profile_children(self) -> Self {
        ...
        match type_ {
            ...
            ffi::IteratorType_NOT_ITERATOR_OPTIMIZED => {
                let ptr = self.into_raw().as_ptr();
                // SAFETY: type tag guarantees this is a NotIteratorOptimized.
                let ni = unsafe { &mut *(ptr as *mut NotIteratorOptimized) };
                if !ni.child.is_null() {
            ...
            ffi::IteratorType_OPTIONAL_ITERATOR => {
                // SAFETY:
            ...
            ffi::IteratorType_OPTIONAL_OPTIMIZED_ITERATOR => {
                let ptr = self.into_raw().as_ptr();
                // SAFETY: type tag guarantees this is an OptionalOptimizedIterator.
                let oi = unsafe { &mut *(ptr as *mut OptionalOptimizedIterator) };
                if !oi.child.is_null() {
                    // SAFETY: child pointer is non-null (just checked).
            ...
            ffi::IteratorType_INTERSECT_ITERATOR => {
                let ptr = self.into_raw().as_ptr();
            ...
            ffi::IteratorType_UNION_ITERATOR => {
                ...
                let ui = unsafe { &mut *(ptr as *mut UnionIterator) };
            ...
            ffi::IteratorType_HYBRID_ITERATOR => {
                let ptr = self.into_raw().as_ptr();
                // SAFETY: type tag guarantees this is a HybridIterator.
                let hi = unsafe { &mut *(ptr as *mut HybridIterator) };
                if !hi.child.is_null() {
                    // SAFETY: child pointer is non-null (just checked).
            ...
            ffi::IteratorType_OPTIMUS_ITERATOR => {
                let ptr = self.into_raw().as_ptr();
                // SAFETY: type tag guarantees this is an OptimizerIterator.
                let oi = unsafe { &mut *(ptr as *mut OptimizerIterator) };
                if !oi.child.is_null() {
                    // SAFETY: child pointer is non-null (just checked).
                    let child_ptr = unsafe { NonNull::new_unchecked(oi.child) };
                    // SAFETY: child is a valid, owning pointer.
            ...
            ffi::IteratorType_WILDCARD_ITERATOR
            ...
            | ffi::IteratorType_EMPTY_ITERATOR
            | ffi::IteratorType_ID_LIST_SORTED_ITERATOR
            | ffi::IteratorType_ID_LIST_UNSORTED_ITERATOR
            | ffi::IteratorType_METRIC_SORTED_BY_ID_ITERATOR
            | ffi::IteratorType_METRIC_SORTED_BY_SCORE_ITERATOR => self,
            ffi::IteratorType_PROFILE_ITERATOR => {
                unreachable!("profile_children called on an already-profiled iterator")
            }
            _ => unreachable!("Unexpected iterator type: {type_}"),
        }
    }
}
