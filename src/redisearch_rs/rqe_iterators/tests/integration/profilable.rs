...
use rqe_iterators::{
    Intersection, RQEIterator, Wildcard, not::Not, optional::Optional, profile::Profilable,
};
...
fn intersection_wraps_children_and_self() {
    ...
    let inter = Intersection::new(vec![a, b]);
...
fn intersection_empty_children() {
    let inter: Intersection<Mock<0>> = Intersection::new(vec![]);
    let mut profiled = inter.into_profiled();
